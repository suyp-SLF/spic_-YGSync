 ...
 * @author ZXR
 */
public class ElecreceiptTask extends AbstractTask {
	...
	private void adjust(String strURL, String jsonStr, Map header, Map body, Map body1, List<Map> list)
		...
		if (null != jsonObject) {
			...
			if (null != BODY && !"".equals(BODY.get("rowCount")) && !"".equals(BODY.get("pageSize"))
				...
				rowCount = Integer.parseInt((String) BODY.get("rowCount"));// 数据总条数 25
				pageSize = Integer.parseInt((String) BODY.get("pageSize"));// 分页大小10
	...
	private JSONObject doExcute(String strURL, String jsonStr, Map header, Map body, List<Map> list) throws Exception {
		...
		LogUtils.log(false, "HttpClientUtils.postjson1", "正在传输", jsonString, jsonStr, new Date(), null);
		jsonStr = HttpClientUtils.postjson(sikuURL, header, jsonString);
		LogUtils.log(false, "HttpClientUtils.postjson2", "正在传输", jsonString, jsonStr, new Date(), null);
		if (jsonStr != null) {
	...
	private List<ResultEntity> excute(List<Map> list, String isType) throws Exception {
		...
		if ("A".equals(isType)) {// 保存财务公司 的电子回单
			...
			mapBasicData = SelectUtils.loadAllPRO(mapBasicData, filterEntities);
			...
			for (Map data : list) {
				...
				if (null != data.get("FTPURL")) {// 如果txName字段里有url地址 将pdf下载
					...
					if (null == returnMap) {
						LogUtils.log(false, "returnMap1", "正在传输", data.toString(), (String) data.get("FTPURL"),
								new Date(), null);
						continue;
					}
			...
			list.forEach(data -> {
				...
				filterEntities.add(new FilterEntity("bd_currency", "number", (String) data.get("currencyNo")));// 币别
				filterEntities.add(new FilterEntity("bd_accountbanks", "number", (String) data.get("accountNo"), null,
						"number,bankaccountnumber"));// 银行账户
			...
			for (Map data : list) {
				...
				if (null != data.get("FTPURL")) {// 如果txName字段里有url地址 将pdf下载
					...
					// 下载FTP文件，获得输入流和大小
					Map<String, Object> returnMap = downloadFtp((String) data.get("FTPURL"));
					if (null == returnMap || null == returnMap.get("fileSize")
							|| null == returnMap.get("inputStream")) {
						LogUtils.log(false, "returnMap1", "MC中没获取到司库地址配置文件", "司库", "", null, null);
						continue;
					}
					// Map<String, Object> returnMap = downloadFtp("\\111aaa\\111.pdf");
					// 创建附件变量
					...
					LogUtils.log(false, "returnMap2", "MC中没获取到司库地址配置文件", "司库", "", null, null);
	...
	public void financeCompany() {
		...
		try {
			...
			if (null != accountbanks) {
				for (DynamicObject accountbank : accountbanks) {
					...
					try {
						...
						// 保存操作异常
						LogUtils.log(false, "fcElecreceipt", "司库" + ADJUSTERROR_STR,
			...
			LogUtils.log(false, "fcElecreceipt", ADJUSTERROR_STR, "司库", "", startDate, eAll);
	...
	public void bank() {
		...
		try {
				...
				for (DynamicObject accountbank : accountbanks) {
					...
					try {
						...
						try {
							...
							exception.append("\n司库调用失败 账号" + bankaccountnumber + ":\t" + e1.toString());
							// 接口访问失败 后台打印日志
					...
					try {
						...
						exception.append(
								"\n执行保存失败 账号" + bankaccountnumber + ":\t" + eSave == null ? "" : eSave.toString());
						// exception.append("账号" + bankaccountnumber + ":"
						// +eSave==null?"":eSave.toString());
						// 保存操作异常
						LogUtils.log(false, "bankElecreceipt", "司库" + ADJUSTERROR_STR,
			...
			LogUtils.log(false, "bankElecreceipt", ADJUSTERROR_STR, "司库", "", startDate, eAll);
			LogBillUtils.modifyLog(logResult, "2", "其他错误:" + eAll == null ? "" : eAll.toString(), "司库");
		}
